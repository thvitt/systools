...
This repository is intended for small scripts and utilities that do not fit
anywhere else. Scripts marked with † are not in active use and maintenance.

Python scripts reside in python submodules in `tv_systools`. All other scripts
live in `scripts`. The easiest way to install everything is probably using pipx
or uv tools:
...
- `uv tools install git+https://github.com/thvitt/systools`
  ...
  background-labels        Imprint captions on images intended as background --help             Python

  choice                   Randomly select from input lines                                     Python

  clean-sync-conflicts     Remove sync-conflicts files as they are created                      Python
                           from Syncthing                                                       

  data-url                 Convert a file to a data: URL                                        Shell
  ...
  linked-assets            ZIP or copy a HTML file with all local files      --help             Python
                           linked from there                                                    
  ...
  pdfshrink                try to reduce PDF files' file size (using         --help             Python
                           ghostscript)                                                         

  pipxtouvtools            generate a script with `uv tool install` commands                    Python
                           for current pipx setup                                               

  pmvn                     Run Maven in the nearest ancestor directory with                     Shell
                           a POM                                                                

  public-link              *very* specific tool to create a public link for                     Python
                           a file in my nectcloud                                               
  ...
  mimecorrtype             Autodetects and fixes e-mail attachment MIME      †                  Perl
                           types                                                                
...
In certain circumstances (suspend etc.), these connections may become stale,
causing all *future* ssh connections to the relevant hosts to hang. This command
simply looks for the control files and asks ssh to exit the connections. I run
this on every resume via a systemd user service:
